 ...
 * */
static int init_char_device(void); 	/*our own init function (constructor)*/
static void exit_char_device(void);	/*our own exit function (distructor)*/
...
static int init_char_device(void)
{
	...
	ret=alloc_chrdev_region(&mydev,0,1,CHAR_DEVICE_NAME);
