...
{
	printf ("\n LOCK : %d ",pthread_mutex_unlock(&mutex));
	if(pthread_mutex_lock(&mutex)==0)
	{
       		printf("\n In Read thread \n");
        	printf(" String : %s \n",str);
...
int main ()
{
	...
	pthread_mutex_getprioceiling(&mutex,&type);
	printf("\n type : %d ",type);
	pthread_mutexattr_settype(&mutex,PTHREAD_MUTEX_ERRORCHECK_NP);	
	pthread_mutexattr_gettype(&mutex,&type);
	printf("\n type : %d ",type);
