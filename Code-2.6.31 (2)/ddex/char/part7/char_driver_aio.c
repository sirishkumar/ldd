...
static loff_t char_device_lseek(struct file *file,
		            loff_t offset,
			    int orig);
static ssize_t char_device_aio_read(struct kiocb *iocb, const struct iovec *buf,unsigned long count,loff_t pos);

static ssize_t char_device_aio_write(struct kiocb *iocb, const struct iovec *buf,unsigned long count, loff_t pos);
...
static int char_device_defer_op(int write, struct kiocb *iocb,  char __user *buf, size_t count, loff_t pos)
{
        ...
        stuff->iocb = iocb;
        stuff->result = result;
        INIT_WORK(&stuff->work, (work_func_t)char_device_do_deferred_op);
	schedule_delayed_work((struct delayed_work *)&stuff->work, HZ/100);
        return -EIOCBQUEUED;
}

static ssize_t char_device_aio_read(struct kiocb *iocb, const struct iovec *buf,                                      unsigned long  count,loff_t pos)
{
        return char_device_defer_op(0, iocb, (char *)buf, count, pos);
}

static ssize_t char_device_aio_write(struct kiocb *iocb,const struct iovec *buf,unsigned long count, loff_t pos)
{
     return char_device_defer_op(1, iocb, (char *) buf, count, pos);
}
MODULE_AUTHOR("VEDA");
