 ...
 int init_char_device(void)
{
	...
	ret=alloc_chrdev_region(&mydev,0,2,"veda_cdrv");

	char_device_id= MAJOR(mydev);//extract major no
	mydev1 = MKDEV(char_device_id,1);
	...
	device_create(veda_class,NULL,mydev1,NULL,"veda_cdrv1");
#endif	
	...
	ret=cdev_add(veda_cdev,mydev,2);
	if( ret < 0 ) {
